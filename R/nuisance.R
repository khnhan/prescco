...
#' Maximum-likelihood fit of the working model for X | Z
#'
#' Fits a truncated-normal working model for the right-censored covariate
#' \code{X} given optional fully observed covariates \code{Z}.
#'
#' @param w_data,delta_data Observed times and event indicators.
#' @param z_c_data Optional continuous covariates, supplied as a vector or
#'   matrix.
#' @param z_d_data Optional discrete covariates, supplied as a vector or matrix.
#' @param w_min,w_max Truncation bounds for \code{X}.
#'
#' @return A named numeric vector containing the estimated mean-model
#'   coefficients followed by the estimated standard deviation.
#'
#' @examples
#' set.seed(1)
#' n <- 200
...
find_alpha1_MLE <- function(w_data, delta_data,
  ...
  if (length(delta_data) != n) {
    stop("delta_data must have length n")
  }
  ...
  log_llhd_one <- function(alpha1, tau1, w, delta, z_c_i, z_d_i) {
    ...
    F_trunc_w <- truncnorm::ptruncnorm(
      w,
      a = w_min,
      b = w_max,
      mean = mu_i,
  ...
  alpha1_hat <- opt$par[seq_len(p_alpha1)]
  tau1_hat <- exp(
    opt$par[p_alpha1 + 1]
  )
...
#' Maximum-likelihood fit of the working model for C | Z
#'
#' Fits a truncated-normal working model for the censoring time \code{C} given
#' optional fully observed covariates \code{Z}.
#'
#' @param w_data,delta_data Observed times and event indicators.
#' @param z_c_data Optional continuous covariates, supplied as a vector or
#'   matrix.
#' @param z_d_data Optional discrete covariates, supplied as a vector or matrix.
#' @param w_min,w_max Truncation bounds for \code{C}.
#' @param verbose If \code{TRUE}, report the optimization message.
#'
#' @return A named numeric vector containing the estimated mean-model
#'   coefficients followed by the estimated standard deviation.
#'
...
find_alpha2_MLE <- function(w_data, delta_data,
  ...
  if (verbose) {
    message("optim (alpha2): ", opt$message)
  }

  alpha2_hat <- opt$par[seq_len(p_alpha2)]
  tau2_hat <- exp(
    opt$par[p_alpha2 + 1]
  )
